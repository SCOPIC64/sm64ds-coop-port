...
// spelling the cutscene sprite body (src/func_ov002_020f2340.cpp) reaches it by.
// RECONCILED AT THE INTRO LANDING: the plain HH alternatename that used to sit
// here was DEFEATED when this branch met the VS work -- the ov075 lane
// materialized its deliberate width bridge (HH callers into the iiii body,
// the WHF3 class) as a DEFINITION of the HH spelling exported by the iiii
// TU's object, and a defined LHS defeats /alternatename silently. The intro
// caller was byte-verified against the ii body, so it gets the R1/R2 remedy:
// a per-source rename in port/CMakeLists.txt (R7, RenderSub=RenderSub_ii for
// that one TU) and the alias below routes the renamed spelling, which nothing
// can ever define. The ov075 callers keep their bridge untouched.
#pragma comment(linker, "/alternatename:?RenderSub_ii@OAM@@SAXPAUOamAttr@@HH@Z=__ZN3OAM9RenderSubEP7OamAttrii")
