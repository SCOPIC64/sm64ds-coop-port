...
extern int data_0209fc48;            /* running cutscene, 0 = none */
...
extern "C" int ContinueKuppaScriptIfNecessary(void);
...
extern "C" int port_level_change_apply(void)
{
    ...
    /* StartFile's request was already consumed to boot the first level.  The
       opening path restages that same level during its first gameplay tick,
       too late for the post-boot check that used to live in walk_window.  If
       it reaches teardown, every freshly spawned opening actor is marked for
       destruction; teardown cannot converge and the script/audio continue
       with an invisible cast.

       A kuppa command 0x0b is not a duplicate request: on the DS it exits the
       current level, reloads it, and ContinueKuppaScriptIfNecessary starts the
       script pointer parked in data_0209fc4c.  The hosted in-process opening
       cannot tear down the still-live Stage safely, but merely consuming the
       request leaves command 0x0b active and reissues both the level and scene
       request every frame.  That is the fa7e32727 regression: hundreds of
       scene releases disturb display state and the opening never advances.

       Perform the reload's script-continuation half here, once, using the
       ROM's own function.  The outgoing cast has reached the command's end;
       the next script takes over without destroying the shared Stage.  Once
       no opening script is active, ordinary deaths and same-level exits use
       the normal transition path below. */
    static unsigned title_transitions_bridged;
    if (port_title_entry_taken() && data_0209fc48 != 0 && want == from) {
        ++title_transitions_bridged;
        data_02092110 = -1;
        const int continued = ContinueKuppaScriptIfNecessary();
        port_scene_request_release("the title entry already booted this level; "
                                   "continuing the opening in process");
        std::fprintf(stderr, "[title-entry] bridged opening same-level "
                             "transition %d (%u); continuation=%d\n",
                     want, title_transitions_bridged, continued);
        return 0;
    }
