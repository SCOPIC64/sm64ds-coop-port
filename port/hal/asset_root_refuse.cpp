...
/* The launcher shows this verbatim in a window, so it is a sentence a player
   can act on rather than a variable name. It still names the file, because the
   person who has to fix a broken install is sometimes us. */
void say_it(const char *wanted)
{
    char text[1024];
    ...
    snprintf(text, sizeof text,
             "Super Mario 64 DS could not start because it was not told where "
             "its game data is.\r\n\r\n"
             "Start the game with play.bat (or the launcher) instead of running "
             "the .exe on its own. Those set the folder the game reads its data "
             "from; the .exe by itself has no default and will not guess one.\r\n\r\n"
