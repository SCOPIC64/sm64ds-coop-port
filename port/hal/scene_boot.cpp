...
{
    ++g_ti_hits[6];
    /* The hosted title cannot complete its asynchronous ov007 teardown, so
       this actor remains on the shared behaviour list after File A hands off
       to the level.  Its selection result remains 3/4/5 and the matched body
       consequently calls StartFile(1, 0) again on every level frame.  On DS
       the scene teardown removes the actor before the Stage runs.  Retire its
       behaviour at the same committed handoff boundary; do not destroy the
       shared actor list and do not touch the opening script or its cast. */
    if (port_title_entry_taken())
        return 1;
    return func_ov007_020cc2cc((char *)s);
}
