 ...
 *
 * All citations are C:\Users\tango\Documents\tangOS-SM64DS\app\src\
...
/* ---- SIXTEEN ROWS: the port's wide VS palette ------------------------------
 *
 * WHAT THE ROM HAS AND WHY IT IS NOT ENOUGH. yoshi_all_16p_pl is FOUR rows,
 * because the cartridge's VS mode is four players and the players are told
 * apart by palette row alone (every seat is forced to Yoshi -- the rule is four
 * lines long and lives in hal/level_boot.cpp). The port runs sixteen. Above
 * four the rows ran out, and the port's answer until now was to let the row
 * follow a slot number truncated to two bits, which status/VS16HOST.md recorded
 * as owed debt and which the owner met in a seven-player match as "player 5
 * looked like a copy of player 1".
 *
 * WHAT THIS ADDS. Sixteen rows, generated by the arithmetic above and nothing
 * else, so the online story is unchanged: no palette travels, every copy of the
 * game computes every seat's sixteen words from the same inputs, and identical
 * inputs give identical bytes. hal/fs_mods.cpp grows the served BMD's palette
 * from 128 bytes to 512 and points the record at them.
 *
 * THE ROM'S OWN FOUR ROWS ARE UNTOUCHED when nobody picked a colour. Slots 0..3
 * with no pick are the cartridge's bytes verbatim -- that is what
 * test_rows16_rom_identity checks -- so a two- or four-player match with no
 * SM64DS_VS_COLORS draws exactly what it has always drawn.
 *
 * WHICH ROW A GENERATED SLOT IS BUILT FROM. rom[s % 4], for every slot,
 * including 0..3. That keeps slots 0..3 doing precisely what they already did
 * (slot s built from row s) and rotates the ROM's four shading families through
 * the wide seats, so seat 4 anchors on the same row as seat 0 and the same pick
 * in either seat gives the same bytes.
 *
 * THE TWELVE DEFAULTS are authored here rather than derived, because there is
 * nothing to derive them from -- the cartridge has four VS Yoshis and no fifth.
 * They are picked to be far apart from each other AND from the ROM's green,
 * red, blue and yellow, and they are run through the same family transform as a
 * player's own pick, so a defaulted seat is shaded like every other seat rather
 * than being a flat colour. test_rows16_distinct is the check that they stay
 * apart: it compares the body ramps of all sixteen rows pairwise. */

struct VsPick {
    int set;            /* 0 = this slot picked nothing */
    u8  body[3];
    u8  shoes[3];
};

/* Slots 4..15. Body colour, then shoe colour. */
static const u8 kVsDefaultBody[12][3] = {
    { 0xff, 0x7a, 0x00 },   /*  4 orange     */
    { 0xff, 0x2e, 0xa6 },   /*  5 magenta    */
    { 0x00, 0xd2, 0xff },   /*  6 cyan       */
    { 0x8c, 0x46, 0xff },   /*  7 violet     */
    { 0xff, 0xff, 0xff },   /*  8 white      */
    { 0x1e, 0x1e, 0x1e },   /*  9 near-black */
    { 0x8c, 0x5a, 0x28 },   /* 10 brown      */
    { 0x00, 0xa0, 0x8c },   /* 11 teal       */
    { 0xff, 0x96, 0xb4 },   /* 12 pink       */
    { 0x50, 0x5a, 0x96 },   /* 13 slate      */
    { 0xb4, 0xb4, 0xb4 },   /* 14 silver     */
    { 0x8c, 0x8c, 0x28 },   /* 15 khaki      */
};
static const u8 kVsDefaultShoes[12][3] = {
    { 0x1e, 0x3c, 0xff }, { 0x00, 0x3c, 0x1e }, { 0x8c, 0x32, 0x00 },
    { 0xff, 0xd2, 0x00 }, { 0xc8, 0x1e, 0x1e }, { 0xc8, 0xc8, 0xc8 },
    { 0x46, 0xd2, 0xff }, { 0x8c, 0x00, 0x8c }, { 0x32, 0x00, 0x64 },
    { 0xff, 0xa0, 0x00 }, { 0x1e, 0x50, 0xc8 }, { 0x5a, 0x00, 0x28 },
};

/* Build all sixteen rows. `rom` is the FOUR rows as the served file has them
 * (the cartridge's, or whatever a local palette mod already wrote over them --
 * this runs last in that chain on purpose, so a player's own mod is the
 * baseline the wide rows are generated from). `picks` is sixteen entries, any
 * of which may be unset. */
static inline void vs_palette_rows16(const u16 rom[4][16], const VsPick *picks,
                                     u16 out[16][16])
{
    for (int s = 0; s < 16; ++s) {
        const u16 *base = rom[s & 3];
        if (picks && picks[s].set) {
            vs_palette_row(base, picks[s].body, picks[s].shoes, out[s]);
        } else if (s < 4) {
            for (int i = 0; i < 16; ++i) out[s][i] = base[i];   /* the ROM's */
        } else {
            vs_palette_row(base, kVsDefaultBody[s - 4], kVsDefaultShoes[s - 4],
                           out[s]);
        }
    }
}
