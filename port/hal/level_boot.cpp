...
/* IDEMPOTENT ON PURPOSE, and gate 31 is why. port_ov009_patch rewrites the
   overlay image's own pointers in place, which is not something that can be
   done twice -- a second pass would rebase already-rebased words. The cached
   `lvl` was there to save the work; it is now load-bearing, because the level
   handoff calls the mount again on every entry to the castle grounds. Any
...
// ---- the level-1 mount, registered against the seam -------------------------
//
// port_ov009_mount already has the shape port_level_mount_register asks for:
// no arguments, returns the host LVL_Overlay, idempotent. This is the whole
// registration for the castle grounds, and it is the template for every level
// the mount stream adds -- one line each, next to the mount itself.
//
// The DS address is asserted rather than assumed. data_02092208[1] is the
// ROM's own answer for level 1 and OV009_LVL_OVERLAY is what this file was
// written against; if a config rename ever moves one, the boot says so here
// instead of mounting the wrong bytes.
extern "C" void port_level_mounts_install(void)
{
    static int done;
    if (done)
    ...
    const unsigned ds = port_level_ds_overlay(1);
    if (ds != (unsigned)OV009_LVL_OVERLAY)
        std::fprintf(stderr, "  [lvl] LEVEL 1 DISAGREES: the ROM's table says "
                     "0x%08x, this file mounts 0x%08x\n", ds,
                     (unsigned)OV009_LVL_OVERLAY);
    port_level_mount_register(1, port_ov009_mount);
}
