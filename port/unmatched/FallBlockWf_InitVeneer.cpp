/* HOST COPY of src/func_ov098_0213a794.cpp -- FallBlockWf's shared (ov098
 * daObjBk base) InitResources body, which its ov015 InitResources tail-calls
 * through the cross-overlay veneer. The matched .cpp cannot be compiled verbatim
 * on the MSVC host: it declares its own `extern "C" int func_ov098_0213a794(
 * char*, char**)` while decl_common.h (which it includes) already declares
 * `extern int func_ov098_0213a794(void*, void*)`, and MSVC's C++ front end
 * rejects two extern "C" declarations that differ only in pointer type (C2733,
 * an overload of an extern-C function) -- a strictness mwccarm does not have.
 *
 * This is the port/unmatched/ host-copy pattern: the body is the matched
 * source's line for line, with only the self-declaration dropped so the two
 * extern-C signatures no longer collide. Not a src edit and not undecompiled
 * code -- the function is fully matched, it just does not survive the host C++
 * front end unchanged.
 */
#include <cstdint>
...
void _ZN13RaycastGround12SetObjAndPosERK7Vector3P5Actor(void *self, void *pos,
                                                        void *actor);
int _ZN13RaycastGround10DetectClsnEv(void *self);
void _ZN13RaycastGroundD1Ev(void *self);
void func_ov098_0213a8ec(void *);   /* the collider update callback */
}
...
extern "C" int func_ov098_0213a794(char *self, char **fp)
{
    ...
    _ZN18MovingMeshCollider7SetFileEP8KCL_FileRK9Matrix4x35Fix12IiEsR10CLPS_Block(
        self + 0x124, _ZN12MeshCollider8LoadFileER13SharedFilePtr((void *)fp[1]),
        self + 0x2ec, 0x199, *(short *)(self + 0x8e), (void *)fp[2]);
    func_020393c4((int *)(self + 0x124), (int)(std::intptr_t)&func_ov098_0213a8ec);
    v.x = *(int *)(self + 0x5c);
    y = *(int *)(self + 0x60);
    v.y = y;
    v.z = *(int *)(self + 0x64);
    ...
    *(int *)(self + 0xa0) = -0xc8000;
    r2 = 1;
    *(short *)(self + 0x33a) = 4;
    *(unsigned char *)(self + 0x340) = (unsigned char)r2;
