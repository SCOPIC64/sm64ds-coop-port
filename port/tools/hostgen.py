...
def main():
    ...
    ap.add_argument("--source", action="append", default=[],
                    help="repo-relative promoted source path to transform")
    ...
    if args.all:
        ...
        for rel in args.source:
            hit = (decomp / rel).resolve()
            try:
                hit.relative_to(src.resolve())
            except ValueError:
                sys.exit(f"source is outside decomp src/: {rel}")
            if not hit.is_file() or hit.suffix not in (".c", ".cpp"):
                sys.exit(f"not found in decomp src/: {rel}")
            targets.append(hit)
        for name in args.symbols:
            ...
            targets.append(hit)
        if not targets:
            sys.exit("nothing to do -- pass symbols, --source, or --all")
