...
def cross_mode(out_path, map_paths):
    ...
    for ov, (s, e) in windows.items():
        for ov2, (s2, e2) in windows.items():
            if ov2 != ov and s2 < e and s < e2:
                contested.add(ov)
    if contested:
        print(f"  cross: {', '.join(sorted(contested))} share a DS window, "
              f"dropped as cross targets")

    claims, dropped = [], []
    for m in mounts:
        into = dropped if m["overlay"] in contested else claims
        for a, sz, n in m["provides"]:
            into.append((a, a + sz, n))
    covering = make_covering(claims)
    would_have = make_covering(dropped)
    ...
    for m in mounts:
        ...
        for name, off, target in m["wants"]:
            ...
            if hit is None:
                unresolved += 1
                if would_have(target) is not None:
                    contested_hits += 1
                continue
...
def main():
    ...
    if sys.argv[1] == "--cross":
        cross_mode(pathlib.Path(sys.argv[2]),
