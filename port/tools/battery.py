...
SCENE_BLOCKED = {
    1: ("the ov007 implicit-r0 argument at func_ov007_020ae558 "
        "(a HOST ABI seam)",
        "SCENE 1 BLOCKED: func_ov007_020add3c calls func_ov007_020ae558 "
        "without its implicit r0 argument",
        "dScDSMT_c::InitResources cannot finish. THIS ROW HAS NOW BEEN "
        "CONVERTED THREE TIMES and the history is most of what it is worth, "
        ...
        "THE BLOCKER NOW IS THE SAME DEFECT CLASS ONE FRAME DEEPER, which is "
        "the most useful thing this row records. It is an IMPLICIT r0 "
        "ARGUMENT. src/func_ov007_020add3c.c declares `extern int "
        ...
        "THE MARKER IS ARMED BY A PREDICATE and the predicate learned "
        "something from the last one. PC2's read only the source declaration, "
        "which a displacement does not touch, so taken literally after a seat "
        "it would have kept asserting a blocker that no longer shipped. Both "
        "markers now read TWO facts: the declaration in src/ AND whether the "
        "TU carrying it is still in the port's ov007 slice. Seating this seam "
        "disarms the marker in the configure that notices, and putting a "
        "displaced TU back re-arms it unasked. "
        "WHAT THIS MARKER DOES NOT PROVE, the same sentence the last one "
        "carried and for the same reason: it is armed on ENTERING "
        "InitResources, not on reaching the seam, because every frame between "
        "ti_init and the fault is a matched TU and the port owns none of them. "
        "A different fault inside InitResources would still carry it. "
        "THE SEAT IS DERIVED AND DELIBERATELY NOT TAKEN. Of the two frames, "
        "the one where information is DESTROYED is the CALLER "
        "func_ov007_020add3c: it owns the value and does not pass it, while "
        "the callee is blameless and could not invent it. That is the opposite "
        "frame from the last seam, where the middle body was the one to "
        "displace, and it is why this is a fresh derivation and not the same "
