...
SCENE_BLOCKED = {
    1: ("the ov007 decomp (func_ov007_020b2998 is UNMATCHED)",
        "UNMATCHED ov007 body entered: func_ov007_020b2998",
        "dScDSMT_c::InitResources cannot finish, and the blocker is a HOLE IN "
        "THE DECOMP rather than anything the port does. THIS ROW WAS "
        "CONVERTED, NOT WRITTEN FRESH, and the history is most of what it is "
        "worth. Lane L2 seated the class and recorded the blocker as "
        "func_ov007_020c9688 (0x020c9688, 768 bytes): InitResources reached it "
        "through func_ov007_020b7138 -> func_ov007_020b68e8 -> "
        "func_ov007_020b6d40, hal/scene_boot.cpp's trap returned 0 six times, "
        "and func_ov007_020ade58 dereferenced one of them at +0xc, FAULT "
        "c0000005 accessing 0x0000000c. "
        "RUN LINK60 LANE SC1 RETIRED THAT ONE WITHOUT A DECOMP. "
        "port/unmatched/Ov007_OamCellBank_020c9688.cpp is a host "
        "transcription of the ROM's 192 instructions at that address, read at "
        "the config-aligned base out of extracted/overlays/overlay_0007.bin. "
        "It is not a match and is not counted as one; it is the port's "
        "established interim shape and it retires itself the day src/ has the "
        "body. With it in, that whole subtree completes and InitResources "
        "walks on to its NEXT call. "
        "THE FRONTIER NOW, symbolized out of walk_window.map: "
        "func_ov007_020cc4c0 +0xcf -> func_ov007_020b7090 +0x70 -> "
        "func_ov007_020aed98 +0x66 -> func_ov007_020b066c +0x124, FAULT "
        "c0000005 accessing 0x00000094 with eax 0. The instruction is "
        "`mov eax,[data_ov007_0210342c + 0xF4]` then a store to [eax+0x94], so "
        "the scene state's +0xF4 field is null. It is null for a named reason: "
        "src/func_ov007_020b69c4.c ZEROES it during setup, no matched TU "
        "anywhere writes it back, and the ROM body that fills it is "
        "func_ov007_020b2998 (0x020b2998, 0x23c bytes), which is UNMATCHED and "
        "trapped. Its own instruction says so: 0x020B29C4 "
        "`str r0, [r2, #0xf4]` with r0 the return of func_ov007_020b2bd4 and "
        "r2 the scene state, read off overlay_0007.bin. So the shape is "
        "identical to the row this replaces, one address further on. "
        "WHAT IS PROVEN ANYWAY, and it is more than before: the class "
        "registers, Scene::SetSceneToSpawn -> Scene::SpawnIfNecessary -> "
        "func_02043098 spawns it through the ROM's own spine, the factory "
        "runs, the object comes back as the scene tree ROOT, slot 0 is "
        "DISPATCHED, and InitResources now clears its archive teardown, its "
        "graphics bring-up, Scene::SetFaders and the whole "
        "func_ov007_020b7138 subtree before dying. SM64DS_SCENE_SLOT0=0 was "
        "tried as a skip and refused: with InitResources no-op'd the Render "
        "faults instead (func_ov007_020cc2b0 -> func_ov007_020b7040 -> "
    ...
    0x176: ("the NitroSDK open-by-name file system (data_020a804c is null)",
        "MINIGAME BLOCKED: the NitroSDK open-by-name archive at "
        "data_020a804c is null",
        "dScMgCurling_c, the first minigame. THIS ROW WAS CONVERTED, NOT "
        "WRITTEN FRESH, and that history is most of what it is worth. Lane "
        "MG2 seated the class and recorded the blocker as the arm9 fader: "
        "dScMgBase_c's slot 1 handed data_0209f61c to Scene::SetFaders, the "
        "object's vptr was zero because nothing in the port ran its "
        "constructor, and the run died at "
        "_ZN5Scene9SetFadersEP15FaderBrightness+0x24 accessing 0x00000024. "
        "RUN LINK60 LANE FDR RETIRED THAT. port/hal/fdr_arm9_fader_seat.cpp "
        "runs the ROM's own src/__sinit_02074f80.c at C++ static-init time, "
        "which is where the DS runs it: func_0202fc40 installs the vptr and "
        "func_020731dc registers the destructor, against a host fill of "
        "data_020926f0, the ten-slot vtable that initialiser installs. Slot 1 "
        "now runs to completion. "
        "ONE CORRECTION TRAVELS WITH THE CONVERSION: the object is a "
        "dWipe_c, not a FaderBrightness. The ROM's RTTI at 0x020926dc reads "
        "7dWipe_c over the chain dFdColor_c -> dFdBrightness_c -> dFader_c, "
        "so FaderBrightness IS dFdBrightness_c, the GRANDBASE two levels up, "
        "with dFader_c a third level beyond it. Every document that named the "
        "CLASS -- this row included, MG2's report, "
        "port/mg_fanout_costs.txt section 10 -- had it wrong, and every "
        "document that named the ADDRESS had it right. "
        "WHAT BLOCKS IT NOW is one seam further in and is not a fader, a "
        "minigame or a scene. dScMgCurling_c::InitResources opens a file BY "
        "NAME through the ROM's own NitroSDK file system: "
        "func_ov006_020e3578 -> func_ov004_020adc74 -> func_020182bc -> "
        "func_02018e3c -> func_0205d644 -> func_0205d714 -> func_0205cdf4. "
        "func_0205d714 copies data_020a804c into the FSFile as its archive, "
        "that record is zeroed host storage (hal/scene_boot.cpp:408, where it "
        "is sized three words and read as a VRAM bank record), its only ROM "
        "writer is func_0205cc80's archive registration, and nothing in the "
        "port runs it. func_0205cdf4 then reads `list + 0x10` off the null: "
        "FAULT c0000005 at func_0205cdf4+0x22 accessing 0x00000010. No other "
        "port file load goes this way -- hal/fs.cpp hosts the ID-based seam, "
        "and open-by-name has had no caller until this scene -- so retiring "
        "it is a file-system job rather than a minigame one. The marker "
        "string above is printed by the successor pre-flight in "
        "hal/scene_mg.cpp, beside the retired fader one, which now stands as "
        "a regression assertion. port/fader_boot_map.txt carries the fader "
        "seat's closure, the three dWipe_c vtable slots still standing as "
        "named traps, and this successor's evidence."),
}
