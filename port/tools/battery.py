...
SCENE_BLOCKED = {
    1: ("the ov007 ride-through at func_ov007_020be980 (a HOST ABI seam)",
        "SCENE 1 BLOCKED: the ov007 ride-through at func_ov007_020be980 drops "
        "i and a2",
        "dScDSMT_c::InitResources cannot finish. THIS ROW HAS NOW BEEN "
        "CONVERTED TWICE and the history is most of what it is worth, because "
        "each conversion retired a real blocker. "
        "ONE: lane L2 seated the class and recorded the blocker as "
        "func_ov007_020c9688 (0x020c9688, 768 bytes), reached through "
        "func_ov007_020b7138 -> func_ov007_020b68e8 -> func_ov007_020b6d40, "
        "with hal/scene_boot.cpp's trap returning 0 six times and "
        "func_ov007_020ade58 dereferencing one of them at +0xc. Run link60 "
        "lane SC1 retired that one WITHOUT a decomp: "
        "port/unmatched/Ov007_OamCellBank_020c9688.cpp is a host transcription "
        "of the ROM's own instructions at that address, it is not a match and "
        "is not counted as one, and it retires itself the day src/ has the "
        "body. "
        "TWO: SC1 converted the row to func_ov007_020b2998 (0x020b2998, 0x23c "
        "bytes), the UNMATCHED and trapped writer of the scene state's +0xF4 "
        "field, whose absence left `mov eax,[data_ov007_0210342c + 0xF4]` "
        "loading null and the store to [eax+0x94] faulting. RUN LINK60 LANE "
        "CK1 MATCHED IT (decomp main db0c4960635e, PR #1536) and lane PC2 "
        "brought the TU across by address. The +0xF4 write happens now, the "
        "old fault is GONE, and this row is the frontier behind it. "
        "THE BLOCKER NOW IS NOT A MISSING BODY, and that is what makes it a "
        "different kind of row. It is an ARM REGISTER RIDE-THROUGH across "
        "three matched TUs, all three byte-faithful, that MSVC's stack ABI "
        "cannot carry. src/func_ov007_020b0a20.c calls "
        "func_ov007_020be980(scene->f_100, 1, 0). src/func_ov007_020be980.c "
        "DEFINES that function as (int r0) and declares its own callee as "
        "`extern int func_ov007_020bc3dc(int)`. src/func_ov007_020bc3dc.c "
        "DEFINES that one as (void *c, int i, int a2). On ARM the middle frame "
        "never touches r1 or r2, so the ROM's own `mov r1, #1` and `mov r2, r6` "
        "at 0x020B0A78 and 0x020B0A80 ride straight through it into the "
        "callee's `ldr ip, [r3, r1, lsl #2]`. On the host every argument is a "
        "stack slot: the middle frame pushes one and the callee reads three, "
        "so i and a2 are whatever its own frame happened to hold. "
        "MEASURED, NOT ARGUED, on this tree's own binary: the caller pushes "
        "ebx then 1 then the scene pointer and calls the middle frame, the "
        "middle frame pushes ONE and cleans 4, and the callee faults at +0xd "
        "on `mov ecx,[eax+esi*4]` with eax the array base and esi the garbage "
        "index. The fault address is that arithmetic exactly, so nothing here "
        "is inferred. "
        "THE MARKER IS ARMED BY A PREDICATE, NOT BY A STRING SOMEBODY "
        "REMEMBERS. The PC2 block in port/CMakeLists.txt reads the middle TU's "
        "own one-parameter declaration out of the source and defines "
        "PORT_OV007_RIDETHROUGH_UNSEATED, and hal/scene_boot.cpp's ti_init "
        "prints the marker only under it. Fix the seam and the marker leaves "
        "the build in the configure that notices, proven both ways in a "
        "throwaway configure. "
        "WHAT THIS MARKER DOES NOT PROVE, said here rather than discovered "
        "later: it is armed on ENTERING InitResources, not on reaching the "
        "seam, because every frame between ti_init and the fault is a matched "
        "TU and the port owns no code in between. A different fault inside "
        "InitResources would still carry it. The trap print this replaces was "
        "tighter because the trap WAS the blocker, and tightening this one "
        "means seating the seam, not rewording the string. "
        ...
        "graphics bring-up, Scene::SetFaders, the whole func_ov007_020b7138 "
        "subtree AND the widget setup at func_ov007_020b2998 before dying two "
        "call levels deeper than it did. SM64DS_SCENE_SLOT0=0 was tried as a "
        "skip and refused: with InitResources no-op'd the Render faults "
        "instead (func_ov007_020cc2b0 -> func_ov007_020b7040 -> "
