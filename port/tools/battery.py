...
SCENE_BLOCKED = {
    ...
    0x176: ("the unseated dWipe_c motion slots (fader, not the file system)",
        ...
        "WHAT BLOCKS IT NOW is the fader again, and specifically the part "
        "port/fader_boot_map.txt section 7a wrote down as a hazard it did not "
        "own. The scene reaches its FIRST BEHAVIOUR TICK, which it had never "
        "done: func_02043288 -> mb_bbeh -> func_ov004_020b0620 (dScMgBase_c "
        "slot 7) -> Scene::BeforeBehavior, whose tail is "
        "`ecx = data_0209f5bc; eax = *ecx; call [eax + 0x14]` -- a CDECL call "
        "through a hand-written struct of function pointers into a table whose "
        "three motion slots hold hal/fdr_arm9_fader_seat.cpp's NAMED TRAPS "
        "rather than the ROM's bodies. Slot 0x0c (SetBackwardTime, ROM body "
        "func_0202f928) prints its trap line and control then goes to "
        "0x307FB114, the actor object's own address, with no pushed return on "
        "the stack -- a `ret` onto a stack word, which is the shape a "
        "calling-convention imbalance makes and which 7a predicts. FDR's first "
        "unproven item said those three slots had never been dispatched; they "
        "have now. "
