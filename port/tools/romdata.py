...
def named_entries(root, syms):
    ...
    for name in NAMED:
        ...
        if name in addr_of:
            ...
            # symbols.txt renames entries over time (data_02090e80 is now
            # ACTOR_DEBUG_NAMES); a data_ address always still resolves.
            m = re.match(r"data_([0-9a-fA-F]{8})$", name)
            if m:
                a = int(m.group(1), 16)
            else:
                # coined rows upstream symbols.txt no longer carries at
                # all (ROM addresses, stable across refactors).
                a = {
                    "Camera_SpawnInfo": 0x02086D78,
